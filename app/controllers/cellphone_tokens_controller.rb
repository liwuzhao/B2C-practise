class CellphoneTokensController < ApplicationController
  ...
  def create
    ...
    if User.find_by cellphone: params[:cellphone]
      render json: {status: 'error', message: "手机号已被注册"}
      return
    end

    if session[:token_create_at] and session[:token_create_at] + 60 > Time.now.to_i
    ...
    SendSMS.send params[:cellphone], "注册验证码是#{token}"
    session[:token_create_at] = Time.now.to_i
